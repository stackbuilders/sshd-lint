...
spec =
  describe "normalizing and validating settings" $ do
    ...
    describe "recommendations" $ do
      ...
      it "doesn't allow more than one allowed value at the time" $
        recommendations (Map.fromList [("PermitRootLogin", [["no"], ["without-password"]])])
          (Map.fromList [("PermitRootLogin", ["no", "without-password"])])
          `shouldBe` ["permitrootlogin should be [[\"no\"],[\"without-password\"]], found [\"no\",\"without-password\"]"]
